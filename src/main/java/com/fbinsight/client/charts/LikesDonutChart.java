 ...
 * @author Slobodan
 */
public class LikesDonutChart extends BaseChart {
	...
	void createNewChart(ArrayList<Item> result) {
		chartPanel.clear();
		createDonutChart();
		setChartData(result);
	...
	public Chart createDonutChart() {
		chart = new Chart()
				...
				.setChartTitleText("Likes by type share")
	 ...
	 void setChartData(ArrayList<Item> result) {
	        ...
	        for (Item item : result) {
        		counter++;
        		points[counter] = new Point(item.getType().getFbLabel(), (item.getTotal() * 100) / total);
			}
//	        chart.removeAllSeries();
