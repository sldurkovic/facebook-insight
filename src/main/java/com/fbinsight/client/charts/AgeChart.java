...
public class AgeChart extends BaseChart {
	...
	private void createChart() {
		chart = new Chart()
				...
				.setChartTitleText("Likes by age and gender")
				.setBarPlotOptions(
...
//		map.put("music",  new Number[]{40});
		if(Injector.INSTANCE.getApplication().getToken() != null){
			loading();
			chartPanel.clear();
			Injector.INSTANCE.getService().getAgeStatistics(Injector.INSTANCE.getApplication().getToken(), query, num, new AsyncCallback<Statistics>() {
				
				public void onSuccess(Statistics result) {
					GWT.log(":: age result="+result);
					HashMap<String, Number[]> map = new HashMap<String, Number[]>();
					for (Item item : result.getItems()) {
						map.put(item.getName(), new Number[]{item.getTotal()});
					}
					createNewChart(map);
					stopLoading();
				}
